...
Displays meta data, description box displays a custom scroll bar in most browsers if the description is too long to fit into to the description box.
\begin{figure}[ht!]
  \centering
...
Picture box next and previous buttons are kept the same as the rest of the buttons in the user interface.
\begin{figure}[ht!]
  \centering
...
google.maps.LatLng
\end{verbatim}
This is the latitude and longitude of the marker, which is created using the data that has been returned from the database. The data that is used to create this object is the latitude and longitude of the recording when it first started. 
We decided to use a custom icon for the default marker to give a more consistent theme, also in order to differentiate between non-selected and selected markers a separate custom selected marker was used. This was done by creating the object:
\begin{verbatim}
google.maps.Icon
\end{verbatim}
