...
{
	name=CSS,
	description={Short for Cascading Style Sheets, a new feature being added to \gls{HTML} that gives both Web site developers and users more control over how pages are displayed}
}
...
{
	name=Django,
	description={A free and open source web application framework, written in \gls{Python}, which follows the model–view–controller architectural pattern.}
}
...
{
	name=PIP,
	description={A package management system used to install and manage software packages written in \gls{Python}}
}
...
{
	name=AJAX,
	description={Asynchronous \gls{JavaScript} and XML, it is a term that describes a new approach to using a number of existing technologies together.}
}
...
The Buzz! library supports a number of audio file formats. After reviewing the alternatives at the start, we agreed to use the '.ogg' audio extension as it provides good audio compression without loosing audio quality. We needed to implement a file conversion function in order to create an '.ogg' file from the '.aac' files the mobile application was saving. This was achieved by implementing a \gls{Python} function using a subprocess call and executing a bash command in the shell calling the avconv software with 'input', 'output' and 'conversion codec to be used' parameters:
...
\end{verbatim}
is called in order to play the next recording. The idea is that the waiting time of the worker thread before playing a file depends on the difference in recording start times.
...
% Question 3 - Statistical Results
\newpage
\begin{figure}[ht!]
\centering
...
\end{figure}
% Question 4 - Statistical Results
\begin{figure}[ht!]
\centering
...
% Question 5 - Statistical Results
\newpage
\begin{figure}[ht!]
\centering
...
\end{figure}
% Question 6 - Statistical Results
\begin{figure}[ht!]
\centering
